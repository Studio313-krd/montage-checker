    ...
    public void Write_CreatesTypedAndFormattedFiveSheetWorkbook()
    {
        ...
        var data = new ExcelReportData(
            ...
            [new ExcelSummaryRow(
                ...
                28_800, 25_200, 21_600, 3_600, 0, 0, 1_800, 900, 0, 12)],
            ...
            [new ExcelApplicationRow(
                employeeId, "Иван Петров", new DateOnly(2026, 9, 7), "Adobe Premiere Pro",
                ApplicationClassification.Productive, 21_600)],
            [new ExcelRenderRow(
                employeeId, "Иван Петров", ProcessingType.Render, "Adobe Media Encoder", Start.AddHours(4),
                Start.AddHours(5), @"D:\render\film.mp4", 94, "Высокая загрузка CPU")],
            [new ExcelIdleRow(employeeId, "Иван Петров", Start.AddHours(6), Start.AddHours(6.5))]);
        ...
        Assert.Equal("Количество снимков", summary.Cell("N1").GetString());
        ...
        Assert.Equal(12, summary.Cell("N2").GetValue<int>());
        ...
        Assert.Equal("Рендер", renders.Cell("B2").GetString());
        Assert.Equal(0.94, renders.Cell("H2").GetDouble(), 8);
        Assert.Equal("0%", renders.Cell("H2").Style.NumberFormat.Format);
    }
