...
В Agent `0.10.3` набор расширен до 82 тестов: добавлены серверные часы, сохранение поправки, перевод часов Windows, срок смены и первый heartbeat при неверном локальном времени. API integration дополнительно проверяет отказ события до начала смены, приём с исправленным временем и Online в dashboard.

После исправлений Agent `0.10.2` набор содержит 74 теста. Дополнительные регрессии проверяют сохранение Offline при отложенном повторе/отказе heartbeat, повторный выбор текущей смены после HTTP 428, доставку текущих данных при отказе старой смены, аналогичное поведение скриншотов, HTTP-диагностику без содержимого ответа и обнаружение расхождения часов.
