...
Пользователь получает обычный self-contained GUI `.exe`: без консольного окна и без необходимости выполнять команды. Первый запуск регистрирует задачу Windows Task Scheduler с trigger «при входе пользователя» и restart-on-failure. Физический ПК автоматически регистрируется после выбора отображаемого имени и ввода четырёхзначного пароля, а ежедневная operator session описывает сотрудника, фактически работающего на нём. После 06:00 по часовому поясу компании Agent блокирует сбор полноэкранным окном выбора сотрудника и пароля. Штатный выход после проверки служебного пароля или через **Я НЕ МОНТАЖЕР** завершается с кодом `0` и не перезапускается; crash/аварийное завершение получает ненулевой код и запускается снова.
...
1. Agent обращается к фиксированному production URL, получает список ID/имён активных сотрудников, проверяет выбранный ID и четырёхзначный пароль, регистрирует или повторно использует физический компьютер и получает device-specific credentials.
2. Вход создаёт operator session до следующего 06:00; логины, пароли и прочие данные сотрудников в список выбора не попадают.
...
- Публичный список входа содержит только ID/имя и ограничен 60 запросами в минуту; проверка выбранного сотрудника и четырёхзначного пароля ограничена пятью попытками в минуту по IP, последующая аутентификация привязана к устройству.
- Смена монтажёра проверяет выбранный ID и пароль только после действительного device token и ограничена десятью попытками в минуту по IP; пароль хранится через Data Protection и не сохраняется Agent.
