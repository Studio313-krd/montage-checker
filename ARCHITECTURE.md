...
Пользователь получает обычный self-contained GUI `.exe`: без консольного окна и без необходимости выполнять команды. Первый запуск регистрирует задачу Windows Task Scheduler с trigger «при входе пользователя» и restart-on-failure. Device enrollment описывает физический ПК, а ежедневная operator session — сотрудника, фактически работающего на нём. После 06:00 по часовому поясу компании Agent блокирует сбор полноэкранным выбором имени и PIN. Штатный выход после проверки пароля или через **Я НЕ МОНТАЖЕР** завершается с кодом `0` и не перезапускается; crash/аварийное завершение получает ненулевой код и запускается снова.
...
2. Перед сбором Agent получает список активных сотрудников и создаёт подтверждённую четырёхзначным PIN operator session до следующего 06:00.
...
- Четырёхзначный PIN не заменяет device authentication: он проверяется только после действительного device token, ограничен десятью попытками в минуту по IP и хранится через Data Protection.
...
| Device enrollment отдельно от operator session | Общий ПК можно передавать сотрудникам, а один сотрудник может корректно работать на нескольких ПК |
