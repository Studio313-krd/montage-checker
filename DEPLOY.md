...
При переходе на Agent `0.10.1` сначала обновите Server/SPA через **Pull and redeploy** и убедитесь, что в **Управление → Сотрудники** видны имена и пароли Agent. Только после этого заменяйте EXE на рабочих ПК. Старые Agent `0.9.1` и `0.10.0` продолжают отправлять данные и выбирать сотрудника по прежнему совместимому API, поэтому сервер можно обновить заранее без остановки учёта. Пошаговая замена уже установленных агентов описана в [OPERATOR_SELECTION.md](OPERATOR_SELECTION.md).
