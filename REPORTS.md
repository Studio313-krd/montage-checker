...
`GET /api/reports/summary` отдаёт Total, Productive, Active, Render, Proxy, Background, Idle, Locked и Offline. При работе сотрудника на нескольких ПК человеческие состояния сначала сводятся в единую непересекающуюся шкалу с приоритетом `Active → Idle → Locked → Offline`; один час человека учитывается один раз. Productive — объединение интервалов Human Active и ненормальной машинной работы. Render, Proxy и Background в раздельных колонках являются машино-временем и могут суммироваться по нескольким ПК.
...
В книге всегда создаются `SUMMARY`, `TIMELINE`, `APPLICATIONS`, `RENDERS` и `IDLE`. Даты и длительности являются типизированными значениями Excel, заголовки закреплены, включены фильтры. Детальные листы содержат компьютер; `SUMMARY` разделяет непересекающееся человеко-время и суммарное машино-время. Скриншоты не встраиваются; в сводке присутствует только их количество. Полный контракт описан в [EXCEL.md](EXCEL.md).
