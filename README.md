...
При первом запуске открывается русское окно входа: выберите имя сотрудника в выпадающем списке и введите выданный администратором четырёхзначный пароль. Адрес `https://montage.checker.studio313.ru/` встроен в Agent и не запрашивается. После входа агент остаётся видимым в tray, отправляет heartbeat с текущим foreground-приложением и состоянием idle, а при отсутствии сети сохраняет события в локальную SQLite-очередь. Автозапуск и перезапуск после аварийного завершения настраиваются через Windows Task Scheduler автоматически. Штатный выход защищён паролем `IDDQD` и не вызывает перезапуск.
...
Agent `0.10.3` ежедневно после 06:00 требует выбрать имя сотрудника и ввести четырёхзначный пароль. ЛКМ и ПКМ по значку в tray открывают меню с ручной сменой монтажёра. Порядок работы на общих ПК и безопасное обновление уже установленного `0.9.1`/`0.10.0`/`0.10.1`/`0.10.2` описаны в [OPERATOR_SELECTION.md](OPERATOR_SELECTION.md).

В `0.10.3` время событий и окончание смены отсчитываются от серверного времени: отставание часов Windows больше не вызывает HTTP 428 сразу после успешного входа и повторное открытие окна выбора сотрудника.

Если сотрудник работает, но карточка показывает «Нет связи», см. [TROUBLESHOOTING.md](TROUBLESHOOTING.md). Версия `0.10.2` исправляет зависание отправки после отказа смены, сохраняет достоверный статус и добавляет журнал диагностики.
...
- [OPERATOR_SELECTION.md](OPERATOR_SELECTION.md) — ежедневный вход, общие ПК и обновление старого EXE.
