...
internal sealed class AgentSettingsStore
{
    ...
    public AgentSettings? Load()
    {
        lock (_gate)
        {
            try
            {
                ...
                settings.Clock.RestoreOffset(settings.ServerClockOffsetTicks);
                var changed = false;
    ...
    public void SaveServerTime(AgentSettings settings, DateTimeOffset serverTimeUtc)
    {
        settings.ServerClockOffsetTicks = settings.Clock.Synchronize(serverTimeUtc);
        Save(settings);
    }
    ...
    internal static void SaveOperatorSessionValues(
        ...
        AgentOperatorSessionResponse session)
    {
        ...
        // StartedAtUtc is generated by the server during this successful login,
        // not a client-supplied timestamp. The first event is therefore never older.
        settings.ServerClockOffsetTicks = settings.Clock.Synchronize(session.StartedAtUtc);
    }
