...
import {
  ...
  getUsers,
  revokeAgent,
  regenerateEmployeeAgentPassword,
  updateEmployee,
  updateUser,
} from '../api'
import type {
    ...
    <Modal onClose={onClose} title={employee ? 'Изменить сотрудника' : 'Новый сотрудник'}>
      <form className="management-form" onSubmit={(event) => void submit(event)}>
        ...
        <div className="management-switches">
          ...
          {employee && <label><input checked={draft.isActive} onChange={(event) => setDraft({ ...draft, isActive: event.target.checked })} type="checkbox" /><span><strong>Сотрудник активен</strong><small>Неактивный сотрудник не сможет войти в Agent.</small></span></label>}
        </div>
...
export function ManagementPage({ currentUser }: { currentUser: AuthUser }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [nextEmployees, nextPasswords, nextComputers, nextUsers, nextScreenshotSettings, nextPrivacyProcesses] = await Promise.all([
        getManagedEmployees(),
        getEmployeeAgentPasswords(),
        getManagedComputers(),
        getUsers(),
      ...
      setAgentPasswords(Object.fromEntries(nextPasswords.map((item) => [item.employeeId, item.password])))
  ...
  async function handleRevoke(computer: ManagedComputer) {
    if (!computer.agentId || !window.confirm(`Отключить Agent на компьютере «${computer.name}»? Для повторного подключения сотрудник войдёт по логину и паролю.`)) return
  ...
  async function handleRegeneratePassword(employee: Employee) {
    if (!window.confirm(`Создать новый пароль для сотрудника «${employee.name}»? Старый пароль сразу перестанет работать для новых входов.`)) return
    setRegeneratingPassword(employee.id)
    setNotice(null)
    try {
      const result = await regenerateEmployeeAgentPassword(employee.id)
      setAgentPasswords((current) => ({ ...current, [result.employeeId]: result.password }))
      setNotice(`Для сотрудника «${employee.name}» создан новый пароль: ${result.password}`)
    } catch (caught) {
      setError(messageFrom(caught))
    } finally {
  ...
  return (
    <>
      ...
      {tab === 'employees' && <section className="management-surface" role="tabpanel">
        <header><div><p className="eyebrow">Команда</p><h2>Сотрудники</h2><p>Логин и четырёхзначный пароль подтверждают монтажёра на каждом компьютере до следующего 06:00.</p></div><button className="primary-button management-create" onClick={() => setEmployeeEditor(null)} type="button"><Icon name="userPlus" /> Добавить сотрудника</button></header>
        {loading && !employees.length ? <div className="management-loading">Загружаем сотрудников…</div> : employees.length ? <div className="responsive-table management-table"><table><thead><tr><th>Сотрудник</th><th>Логин</th><th>Пароль Agent</th><th>Мониторинг</th><th>Компьютеры</th><th><span className="visually-hidden">Действия</span></th></tr></thead><tbody>{employees.map((employee) => {
          const employeeComputers = computers.filter((computer) => computer.employeeId === employee.id)
          return <tr key={employee.id}><td><strong>{employee.name}</strong><small>{employee.department ?? 'Отдел не указан'}</small>{!employee.isActive && <span className="management-status is-inactive">Неактивен</span>}</td><td><code>{employee.login}</code></td><td><strong className="agent-password">{agentPasswords[employee.id] ?? '••••'}</strong><small>Выдайте сотруднику лично</small></td><td><strong>{employee.screenshotEnabled ? `Снимки · ${employee.screenshotIntervalMinutes} мин` : 'Без снимков'}</strong><small>Простой после {employee.idleThresholdSeconds} сек.</small></td><td><strong>{employeeComputers.length || '—'}</strong><small>{employeeComputers.some((item) => item.agentStatus === 'Online') ? 'Есть Agent в сети' : 'Нет Agent в сети'}</small></td><td><div className="management-row-actions"><button disabled={!employee.isActive || regeneratingPassword === employee.id} onClick={() => void handleRegeneratePassword(employee)} type="button"><Icon name="refresh" size={17} /> {regeneratingPassword === employee.id ? 'Создаём…' : 'Создать новый пароль'}</button><button onClick={() => setEmployeeEditor(employee)} type="button">Изменить</button></div></td></tr>
        })}</tbody></table></div> : <div className="empty-state management-empty"><Icon name="users" size={30} /><h3>Сотрудников пока нет</h3><p>Добавьте первого сотрудника — логин и пароль для Agent появятся автоматически.</p></div>}
      </section>}
