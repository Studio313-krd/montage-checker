...
export function ManagementPage({ currentUser }: { currentUser: AuthUser }) {
  ...
  async function handleRevoke(computer: ManagedComputer) {
    if (!computer.agentId || !window.confirm(`Отключить Agent на компьютере «${computer.name}»? Для повторного подключения сотрудник выберет своё имя и введёт пароль.`)) return
